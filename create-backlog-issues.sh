#!/usr/bin/env bash
# Creates GitHub issues from the product backlog.
# Run locally where `gh` CLI is authenticated.
# Safe to re-run — but will create duplicates if issues already exist.
set -euo pipefail

REPO="danielway/nexrad-workbench"

gh issue create --repo "$REPO" \
  --title "Bug: Timeline interaction and display issues" \
  --body "$(cat <<'BODY'
## Description
...
gh issue create --repo "$REPO" \
  --title "Bug: Real-time streaming display issues" \
  --body "$(cat <<'BODY'
## Description
...
gh issue create --repo "$REPO" \
  --title "Progressive sweep animation and data age visualization" \
  --body "$(cat <<'BODY'
## Description
...
### Continuous (wiper)
At each azimuth/range, the most recent eligible value is shown. As the radar sweeps, new data progressively overwrites older data. The canvas always shows maximum spatial coverage. Lookback is bounded to prevent rendering arbitrarily old data.

### Sweep-isolated
When a new sweep begins, the canvas clears entirely. Data paints in fresh from the starting azimuth. Only data from the current sweep is ever visible — a growing wedge until the sweep completes, then a full circle until the next matching sweep clears it. Guarantees temporal purity.
...
gh issue create --repo "$REPO" \
  --title "Acquisition queue and network activity UI" \
  --body "$(cat <<'BODY'
## Description
...
gh issue create --repo "$REPO" \
  --title "Real-time streaming enhancements" \
  --body "$(cat <<'BODY'
## Description
...
gh issue create --repo "$REPO" \
  --title "Data pipeline parallelization" \
  --body "$(cat <<'BODY'
## Description
...
gh issue create --repo "$REPO" \
  --title "Local file upload support" \
  --body "$(cat <<'BODY'
## Description
...
gh issue create --repo "$REPO" \
  --title "Cell detection and analysis improvements" \
  --body "$(cat <<'BODY'
## Description
...
gh issue create --repo "$REPO" \
  --title "Initial load experience improvements" \
  --body "$(cat <<'BODY'
## Description
...
gh issue create --repo "$REPO" \
  --title "Additional data products" \
  --body "$(cat <<'BODY'
## Description
