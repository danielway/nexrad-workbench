...
mod coverage_tests {
    ...
    // ── tilt_matches: None (Latest/auto) matches every elevation ──────────────

    #[wasm_bindgen_test]
    fn tilt_matches_none_is_wildcard() {
        // None ⇒ every elevation is a frame.
        assert!(tilt_matches(None, 0));
        assert!(tilt_matches(None, 1));
        assert!(tilt_matches(None, 7));
        // Some(t) ⇒ only the exact elevation matches.
        assert!(tilt_matches(Some(3), 3));
        assert!(!tilt_matches(Some(3), 2));
        assert!(!tilt_matches(Some(3), 4));
    }

    // ── range_hits / secs_hit: exact 60s boundary is inclusive, 61 is out ─────

    #[wasm_bindgen_test]
    fn range_hits_boundary_is_inclusive_at_tolerance() {
        // |start - key| == 60 hits; 61 misses; range end is ignored (start only).
        let ranges = [(1_060_i64, 9_999_i64)];
        assert!(range_hits(&ranges, 1_000)); // diff 60 == tolerance
        let ranges2 = [(1_061_i64, 9_999_i64)];
        assert!(!range_hits(&ranges2, 1_000)); // diff 61 > tolerance
                                               // Negative-direction diff also counts (abs).
        let ranges3 = [(940_i64, 9_999_i64)];
        assert!(range_hits(&ranges3, 1_000)); // diff 60
                                              // Empty slice never hits.
        assert!(!range_hits(&[], 1_000));
    }

    #[wasm_bindgen_test]
    fn secs_hit_boundary_and_empty() {
        assert!(secs_hit(&[1_060], 1_000)); // diff 60 inclusive
        assert!(secs_hit(&[940], 1_000)); // diff 60 the other way
        assert!(!secs_hit(&[1_061], 1_000)); // diff 61 out
        assert!(!secs_hit(&[], 1_000)); // empty
                                        // Any one of several matching counts.
        assert!(secs_hit(&[5_000, 1_030, 9_000], 1_000));
    }

    // ── ring_flags: prev_active path + active suppresses prev_active ──────────

    #[wasm_bindgen_test]
    fn ring_flags_prev_active_only_when_not_active() {
        let mut j = empty_join("reflectivity", Some(1));
        // Neither set → both false.
        assert_eq!(ring_flags(&j, 1_000.0, 1), (false, false));
        // prev_active matches, active none → (false, true).
        j.prev_active = Some((1_000.0, 1));
        assert_eq!(ring_flags(&j, 1_000.0, 1), (false, true));
        // active also matches the SAME cell → active wins, prev suppressed.
        j.active = Some((1_000.0, 1));
        assert_eq!(ring_flags(&j, 1_000.0, 1), (true, false));
        // Elevation mismatch → no match even if timestamp matches.
        let mut j2 = empty_join("reflectivity", Some(1));
        j2.active = Some((1_000.0, 2));
        assert_eq!(ring_flags(&j2, 1_000.0, 1), (false, false));
        // Timestamp just outside the 0.5s ring tolerance → no match.
        let mut j3 = empty_join("reflectivity", Some(1));
        j3.active = Some((1_000.6, 1));
        assert_eq!(ring_flags(&j3, 1_000.0, 1), (false, false));
        // Timestamp within 0.5s → match.
        let mut j4 = empty_join("reflectivity", Some(1));
        j4.active = Some((1_000.4, 1));
        assert_eq!(ring_flags(&j4, 1_000.0, 1), (true, false));
    }

    // ── ring_flags_in_live_volume: prev_active + 60s gate by volume_start ─────

    #[wasm_bindgen_test]
    fn ring_flags_in_live_volume_prev_and_gate() {
        let mut j = empty_join("reflectivity", Some(1));
        // active inside the volume (within 60s of volume_start), elev matches.
        j.active = Some((1_000_059.0, 1));
        assert_eq!(ring_flags_in_live_volume(&j, 1_000_000.0, 1), (true, false));
        // Just outside the 60s gate → no ring even with matching elevation.
        let mut j2 = empty_join("reflectivity", Some(1));
        j2.active = Some((1_000_061.0, 1));
        assert_eq!(
            ring_flags_in_live_volume(&j2, 1_000_000.0, 1),
            (false, false)
        );
        // prev_active inside the gate, no active → (false, true).
        let mut j3 = empty_join("reflectivity", Some(1));
        j3.prev_active = Some((1_000_010.0, 1));
        assert_eq!(
            ring_flags_in_live_volume(&j3, 1_000_000.0, 1),
            (false, true)
        );
        // active suppresses prev_active when both match within the gate.
        let mut j4 = empty_join("reflectivity", Some(1));
        j4.active = Some((1_000_010.0, 1));
        j4.prev_active = Some((1_000_020.0, 1));
        assert_eq!(
            ring_flags_in_live_volume(&j4, 1_000_000.0, 1),
            (true, false)
        );
        // Elevation mismatch → no match.
        let mut j5 = empty_join("reflectivity", Some(1));
        j5.active = Some((1_000_010.0, 2));
        assert_eq!(
            ring_flags_in_live_volume(&j5, 1_000_000.0, 1),
            (false, false)
        );
    }
    ...
    fn is_live_scan_none_never_live() {
        ...
        // None live anchor → never live.
        assert!(!is_live_scan(&scan, None));
        // Matching ms → live; non-matching → not.
        let ms = scan.key_ms();
        assert!(is_live_scan(&scan, Some(ms)));
        assert!(!is_live_scan(&scan, Some(ms + 1)));
    }

    // ── clamped_display_end: boundary end floored at the scan's own end_time ──

    #[wasm_bindgen_test]
    fn clamped_display_end_floors_at_scan_end_time() {
        // A boundary whose `end` is EARLIER than the scan's real end_time must
        // never truncate real collected data → result = end_time (the .max).
        let mut scan = cached_scan(1_000.0, Vec::new());
        scan.start_time = 1_000.0;
        scan.end_time = 1_080.0; // real data runs to 1080
        let scans = vec![scan];
        // Archive boundary says next volume starts at 1_050 (earlier than 1080).
        let shadows = vec![ScanBoundary {
            start: 1_000,
            end: 1_050,
        }];
        let de = clamped_display_end(&scans, &shadows, 0);
        assert_eq!(de, 1_080.0); // floored at end_time, not clamped down to 1050
    }

    #[wasm_bindgen_test]
    fn clamped_display_end_no_boundary_no_vcp_is_end_time() {
        // No boundary, no VCP pattern, last scan → display end == end_time.
        let mut scan = cached_scan(1_000.0, Vec::new());
        scan.start_time = 1_000.0;
        scan.end_time = 1_040.0;
        scan.vcp_pattern = None;
        let scans = vec![scan];
        let shadows: Vec<ScanBoundary> = Vec::new();
        assert_eq!(clamped_display_end(&scans, &shadows, 0), 1_040.0);
    }

    // ── merge_cached_into_live: no matching elevation is a no-op; idempotent ──

    #[wasm_bindgen_test]
    fn merge_no_matching_elevation_is_noop() {
        // Live model only has elev 2; cached scan provides elev 5 (not present).
        let mut pos = live_model(vec![sweep_pos(2, SweepProjectionStatus::FutureExpected)]);
        let scan = cached_scan(0.0, vec![cached_sweep(5, 1.0, 2.0)]);
        merge_cached_into_live(&mut pos, &scan);
        // elev 2 untouched (still future), no new sweep added.
        assert_eq!(pos.sweeps.len(), 1);
        assert_eq!(pos.sweeps[0].status, SweepProjectionStatus::FutureExpected);
        assert_eq!(pos.completed_count(), 0);
    }

    #[wasm_bindgen_test]
    fn merge_is_idempotent() {
        let mut pos = live_model(vec![sweep_pos(1, SweepProjectionStatus::FutureExpected)]);
        let scan = cached_scan(0.0, vec![cached_sweep(1, 10.0, 40.0)]);
        merge_cached_into_live(&mut pos, &scan);
        let first = (
            pos.sweeps[0].status,
            pos.sweeps[0].collection_start_secs,
            pos.sweeps[0].collection_end_secs,
        );
        // Running it again changes nothing (already CollectedByUs).
        merge_cached_into_live(&mut pos, &scan);
        assert_eq!(
            (
                pos.sweeps[0].status,
        ...
        assert_eq!(first.0, SweepProjectionStatus::CollectedByUs);
        assert_eq!(first.1, 10.0);
        assert_eq!(first.2, 40.0);
    }

    // ── settled_scan_at: returns the cached scan, None over a gap ─────────────

    #[wasm_bindgen_test]
    fn settled_scan_at_hits_cache_and_misses_gap() {
        let mut s = cached_scan(1_000.0, vec![cached_sweep(1, 1_000.0, 1_030.0)]);
        s.start_time = 1_000.0;
        s.end_time = 1_300.0;
        let cache = RadarTimeline { scans: vec![s] };
        let shadows: Vec<ScanBoundary> = Vec::new();
        let view = TimelineView::build(&cache, &shadows, None, None);
        // Inside the scan span → Some.
        assert!(view.settled_scan_at(1_100.0).is_some());
        assert_eq!(
            view.settled_scan_at(1_100.0).unwrap().key_timestamp,
            1_000.0
        );
        // Before the scan → None.
        assert!(view.settled_scan_at(500.0).is_none());
        // After the scan's end_time → None.
        assert!(view.settled_scan_at(2_000.0).is_none());
    }

    // ── scan_start_at: live volume wins when ts falls inside it ───────────────

    #[wasm_bindgen_test]
    fn scan_start_at_prefers_live_volume() {
        let pos = live_model(vec![sweep_pos(1, SweepProjectionStatus::InProgress)]);
        // volume_start = 1_700_000_000, volume_end = 1_700_000_300.
        let live = crate::state::LiveModeState::with_dummy_streaming(
            crate::state::LivePhase::Streaming,
            1_700_000_010.0,
        );
        ...
        let shadows: Vec<ScanBoundary> = Vec::new();
        let view = TimelineView::build(&cache, &shadows, Some(&live), Some(&pos));
        // A ts inside the live volume → its volume_start.
        assert_eq!(view.scan_start_at(1_700_000_150.0), Some(1_700_000_000.0));
        // Outside the live volume and no cache/shadow → None.
        assert_eq!(view.scan_start_at(1_700_000_400.0), None);
    }

    // ── Latest mode (tilt None): every elevation becomes a frame cell ─────────

    #[wasm_bindgen_test]
    fn join_latest_mode_yields_all_elevations() {
        let cache = RadarTimeline {
            scans: vec![cached_scan(
                1_000.0,
                vec![
                    cached_sweep_products(1, 1_000.0, 1_030.0, &["reflectivity"]),
                    cached_sweep_products(2, 1_030.0, 1_060.0, &["reflectivity"]),
                    cached_sweep_products(3, 1_060.0, 1_090.0, &["reflectivity"]),
                ],
            )],
        };
        let shadows: Vec<ScanBoundary> = Vec::new();
        let view = TimelineView::build(&cache, &shadows, None, None);
        // tilt None → all three sweeps are cells.
        let c = view.frame_containers_in_range(0.0, 5_000.0, empty_join("reflectivity", None));
        assert_eq!(c.len(), 1);
        assert_eq!(c[0].cells.len(), 3);
        assert!(c[0]
            .cells
            .iter()
            .all(|cell| cell.state == FrameCellState::Cached));
    }

    // ── shadow Available cell carries elevation 0 in Latest mode ──────────────

    #[wasm_bindgen_test]
    fn join_shadow_cell_elevation_reflects_tilt() {
        let cache = RadarTimeline { scans: Vec::new() };
        let shadows = vec![ScanBoundary {
            start: 1_000,
            end: 1_300,
        ...
        let view = TimelineView::build(&cache, &shadows, None, None);
        // Tilt Some(4) → the assumed cell carries elevation 4.
        let c = view.frame_containers_in_range(0.0, 5_000.0, empty_join("reflectivity", Some(4)));
        assert_eq!(c[0].cells[0].elevation_number, 4);
        assert_eq!(c[0].cells[0].start_secs, 1_000.0);
        assert_eq!(c[0].cells[0].end_secs, 1_300.0);
        // Tilt None (Latest) → elevation 0.
        let c2 = view.frame_containers_in_range(0.0, 5_000.0, empty_join("reflectivity", None));
        assert_eq!(c2[0].cells[0].elevation_number, 0);
    }

    // ── shadow covered by cache is dropped from the frame containers ──────────

    #[wasm_bindgen_test]
    fn join_shadow_covered_by_cache_is_skipped() {
        // A cached scan at 1_000 and a shadow at 1_030 (within 60s) → the shadow
        // is suppressed; only the cached container remains.
        let cache = RadarTimeline {
            scans: vec![cached_scan(
                1_000.0,
                vec![cached_sweep_products(
        ...
        let view = TimelineView::build(&cache, &shadows, None, None);
        let c = view.frame_containers_in_range(0.0, 5_000.0, empty_join("reflectivity", Some(1)));
        // Only the cached container — the shadow is covered (within tolerance).
        assert_eq!(c.len(), 1);
        assert!(!c[0].is_available);
    }

    // ── is_covered_by_cached includes the live volume in the coverage set ─────

    #[wasm_bindgen_test]
    fn covered_by_cached_includes_live_volume() {
        // volume_start anchor only comes from current_volume; with_dummy_streaming
        // leaves it None, so coverage relies on the cached scans alone here. Build
        // a cache whose scan supplies coverage and confirm the band math.
        let cache = RadarTimeline {
            scans: vec![cached_scan(2_000.0, Vec::new())],
        };
        let shadows: Vec<ScanBoundary> = Vec::new();
        let view = TimelineView::build(&cache, &shadows, None, None);
        assert!(view.is_covered_by_cached(2_000));
        assert!(view.is_covered_by_cached(2_060)); // +60 inclusive
        assert!(!view.is_covered_by_cached(2_061)); // +61 out
    }

    // ── accessors: cache() and shadow_boundaries() return the borrowed sources ─

    #[wasm_bindgen_test]
    fn accessors_return_underlying_sources() {
        let cache = RadarTimeline {
            scans: vec![
                cached_scan(1_000.0, Vec::new()),
                cached_scan(2_000.0, Vec::new()),
            ],
        };
        let shadows = vec![
            ScanBoundary {
        ...
        let view = TimelineView::build(&cache, &shadows, None, None);
        assert_eq!(view.cache().scans.len(), 2);
        assert_eq!(view.shadow_boundaries().len(), 2);
        assert_eq!(view.shadow_boundaries()[1].start, 2_000);
        // No live overlay built (no live_state) → live_volume None.
        assert!(view.live_volume().is_none());
    }

    // ── frame container ordering: settled, then live last among equal starts ──

    #[wasm_bindgen_test]
    fn frame_containers_live_paints_last_at_equal_start() {
        // A cached scan and a live volume sharing the same start second. The live
        // container must sort AFTER the settled one (is_live false < true).
        let cache = RadarTimeline {
            scans: vec![cached_scan(
                1_700_000_000.0,
                vec![cached_sweep_products(
        ...
        let pos = live_model(vec![sp]); // volume_start == 1_700_000_000.0
        let live = crate::state::LiveModeState::with_dummy_streaming(
            crate::state::LivePhase::Streaming,
            1_700_000_010.0,
        );
        ...
        assert_eq!(c.len(), 2);
        // Same start_secs; the settled container comes first, live last.
        assert_eq!(c[0].start_secs, c[1].start_secs);
        assert!(!c[0].is_live);
        assert!(c[1].is_live);
    }

    // ── next-scan ghost → a fully-Projected container ─────────────────────────

    #[wasm_bindgen_test]
    fn join_next_scan_ghost_is_projected_container() {
        let mut s_inprog = sweep_pos(1, SweepProjectionStatus::InProgress);
        s_inprog.collection_start_secs = 1_700_000_000.0;
        s_inprog.collection_end_secs = 1_700_000_030.0;
        s_inprog.chunks_in_sweep = 3;
        let mut pos = live_model(vec![s_inprog]);
        // Attach a next-scan ghost one volume ahead with two matching cells.
        let mut g1 = sweep_pos(1, SweepProjectionStatus::FutureExpected);
        g1.scan_role = ProjectionScanRole::NextScan;
        g1.collection_start_secs = 1_700_000_300.0;
        g1.collection_end_secs = 1_700_000_330.0;
        let mut g2 = sweep_pos(2, SweepProjectionStatus::FutureExpected);
        g2.scan_role = ProjectionScanRole::NextScan;
        g2.collection_start_secs = 1_700_000_330.0;
        g2.collection_end_secs = 1_700_000_360.0;
        let mut ghost = live_model(vec![g1, g2]);
        ghost.volume_start = 1_700_000_300.0;
        ...
        let shadows: Vec<ScanBoundary> = Vec::new();
        let view = TimelineView::build(&cache, &shadows, Some(&live), Some(&pos));
        // tilt None → ghost surfaces both elevations as Projected cells.
        let c = view.frame_containers_in_range(
            1_699_999_000.0,
            1_700_001_000.0,
            empty_join("reflectivity", None),
        );
        // The live volume container + the ghost container.
        let ghost_c = c
            .iter()
            .find(|cc| (cc.start_secs - 1_700_000_300.0).abs() < 0.5)
            .expect("ghost container present");
        assert!(!ghost_c.is_live);
        assert_eq!(ghost_c.cells.len(), 2);
        assert!(ghost_c
            .cells
            .iter()
            .all(|cell| cell.state == FrameCellState::Projected));
        // Ghost sub-texture carries both sweep spans.
        assert_eq!(ghost_c.sweep_spans.len(), 2);
        // None of the ghost cells ring (ghosts are never active).
        assert!(ghost_c.cells.iter().all(|cell| !cell.is_active));
    }

    // ── cached cell mid-refresh (in_flight) without product reads InFlight ────

    #[wasm_bindgen_test]
    fn join_cached_without_product_in_flight_reads_inflight() {
        // A cached scan whose matching tilt lacks the selected product, but whose
        // scan-start overlaps an in-flight range → InFlight (mid-fetch), not
        // Available.
        let cache = RadarTimeline {
            scans: vec![cached_scan(
                1_000.0,
        ...
        // Same scan but its scan-start is in the failed set (no in-flight) → Failed.
        let failed = [1_010_i64];
        let mut j2 = empty_join("reflectivity", Some(1));
        j2.failed = &failed;
        let c2 = view.frame_containers_in_range(0.0, 5_000.0, j2);
        assert_eq!(c2[0].cells[0].state, FrameCellState::Failed);
    }

    // ── empty cached set ("legacy index entry") counts as on-device → Cached ──

    #[wasm_bindgen_test]
    fn join_empty_cached_products_is_cached() {
        // A sweep with NO cached_products (legacy/unknown) still counts as
        // downloaded for cell purposes → Cached.
        let cache = RadarTimeline {
            scans: vec![cached_scan(
                1_000.0,
                vec![cached_sweep(1, 1_000.0, 1_030.0)],
            )],
        };
        let shadows: Vec<ScanBoundary> = Vec::new();
        let view = TimelineView::build(&cache, &shadows, None, None);
    ...
    // ── visual_scans_in_range filters by clamped end vs window start ──────────

    #[wasm_bindgen_test]
    fn visual_scans_culls_when_window_after_block() {
        let mut s = cached_scan(1_000.0, Vec::new());
        s.start_time = 1_000.0;
        s.end_time = 1_050.0;
        s.vcp_pattern = None; // display end == end_time == 1_050
        let cache = RadarTimeline { scans: vec![s] };
        let shadows: Vec<ScanBoundary> = Vec::new();
        let view = TimelineView::build(&cache, &shadows, None, None);
        // Window entirely after the block's clamped end (1_050) → excluded.
        assert_eq!(view.visual_scans_in_range(1_060.0, 2_000.0).count(), 0);
        // Window straddling the block → included, end == 1_050.
        let pairs: Vec<_> = view.visual_scans_in_range(900.0, 1_020.0).collect();
        assert_eq!(pairs.len(), 1);
        assert_eq!(pairs[0].1, 1_050.0);
        // Window entirely before the block's start (start_time 1_000) → excluded.
        assert_eq!(view.visual_scans_in_range(0.0, 900.0).count(), 0);
    }

    // ── tolerance alias constants agree ───────────────────────────────────────

    #[wasm_bindgen_test]
    fn tolerance_constants_agree() {
        assert_eq!(SCAN_JOIN_TOLERANCE_SECS, 60);
        assert_eq!(BOUNDARY_MATCH_TOLERANCE_SECS, SCAN_JOIN_TOLERANCE_SECS);
    }
}
