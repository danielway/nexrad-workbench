    ...
    fn handle_live_decoded_outcome(&mut self, result: nexrad::DecodeResult) {
        ...
        if let (Some(ref renderer), Some(ref gl)) = (&self.gpu.gpu, &self.gpu.gl) {
            if let Ok(mut r) = renderer.lock() {
                ...
                // Pass sorted radials directly to the GPU — same as the archive
                // path. The shader's azimuth search reads actual angles from the
                // azimuth texture rather than assuming uniform spacing, so no
                // padding is needed. The previous padding to 720 slots caused
                // collisions where two radials mapped to the same slot, leaving
                // thin radial gaps in the render.
                r.update_data(
                    gl,
                    &result.azimuths,
                    &result.gate_values,
