...
mod coverage_tests {
    ...
    fn range_hits_boundary_is_inclusive_at_tolerance() {
        ...
        assert!(range_hits(&ranges, 1_000));
        let ranges2 = [(1_061_i64, 9_999_i64)];
        assert!(!range_hits(&ranges2, 1_000));
        let ranges3 = [(940_i64, 9_999_i64)];
        assert!(range_hits(&ranges3, 1_000));
        assert!(!range_hits(&[], 1_000));
    }

    #[wasm_bindgen_test]
    fn secs_hit_boundary_and_empty() {
        assert!(secs_hit(&[1_060], 1_000));
        assert!(secs_hit(&[940], 1_000));
        assert!(!secs_hit(&[1_061], 1_000));
        assert!(!secs_hit(&[], 1_000));
    ...
    fn covered_by_cached_includes_live_volume() {
        ...
        assert!(view.is_covered_by_cached(2_060));
        assert!(!view.is_covered_by_cached(2_061));
    }
